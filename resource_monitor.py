...
class ResourceMonitor:
    """Monitor CPU, memory, and GPU usage during training with thread safety."""
    ...
    def __init__(self, monitor_interval: float = 1.0):
        ...
        # Thread-safe lock for accessing data
    ...
    def _monitor_loop(self):
        """Background monitoring loop (thread-safe)."""
        while True:
    ...
    def get_summary(self) -> Dict:
        """
        Get summary of resource usage (thread-safe).
...
class ExperimentTracker:
    """Track multiple experiments and their metrics with thread safety."""
    ...
    def start_experiment(self, name: str, **metadata):
        """Start tracking a new experiment (thread-safe)."""
        experiment = {
            'name': name,
            'metadata': metadata,
    ...
    def get_experiment_data(self, experiment_idx: int) -> Dict:
        """Get complete experiment data (thread-safe)."""
        with self._lock:
            if experiment_idx >= len(self.experiments):
                return {}
    ...
    def cleanup_all(self):
        """Cleanup all experiments (thread-safe)."""
        with self._lock:
        ...
        for experiment in experiments:
            if 'monitor' in experiment:
                try:
                    ...
                    print(f"Warning: Error cleaning up experiment '{experiment.get('name')}': {e}")
