...
def load_config():
    ...
    # Look for config.json: first in CWD, then next to this module
    cwd_path = os.path.join(os.getcwd(), 'config.json')
    module_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config.json')

    file_path = cwd_path if os.path.exists(cwd_path) else module_path
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Could not find config.json in {os.getcwd()} or {os.path.dirname(os.path.abspath(__file__))}"
        )
